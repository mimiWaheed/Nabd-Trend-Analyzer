...
(async () => {
  ...
  assert(el('dbTrendList') && el('dbTrendList').querySelector('.topic-pie .donut'), 'trending topics must render the share pie donut');
  assert(el('dbTrendList') && el('dbTrendList').querySelectorAll('.topic-pie-row').length === 1, 'topic pie renders 1 legend row');
  assert(el('dbHlList') && el('dbHlList').children.length === 1, 'aiHighlights rendered 1 card');
  assert(el('dbGenNote') && !el('dbGenNote').hidden && txt('dbGenNote').indexOf('resources') !== -1, 'summary gen-note shows generated time + resource count');
  assert(el('regionGrid') && el('regionGrid').querySelector('.region-tile.national'), 'national heatmap tile rendered for Egypt');
  assert(el('regionGrid') && el('regionGrid').querySelectorAll('.region-tile').length >= 20, 'city heatmap renders the full Egyptian city grid');
  assert(el('dbKwList') && el('dbKwList').children.length === 2, 'top keywords rendered 2 rows');
  assert(el('dbKwList') && el('dbKwList').querySelectorAll('.vbar').length === 2, 'top keywords rendered as vertical bars');
  ...
  assert(el('dbPhList') && el('dbPhList').querySelectorAll('.heat-tile').length === 2, 'top phrases rendered as a heatmap grid');
  ...
  assert(el('dbHealthChart') && el('dbHealthChart').querySelector('.radar-chart'), 'signal health rendered as a radar chart');
  ...
  assert(el('dbSrcList') && el('dbSrcList').querySelectorAll('.vbar').length === 1, 'sources rendered as vertical bars');
  assert(el('dbSrcList') && txt('dbSrcList').indexOf('اليوم السابع') !== -1, 'source اليوم السابع visible');
  assert(el('dbActList') && el('dbActList').querySelectorAll('.act-btn').length === 4, 'suggested actions card renders 4 action buttons');

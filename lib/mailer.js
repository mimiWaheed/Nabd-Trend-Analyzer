...
function sendMail({ to, subject, text, html }) {
  ...
  return transport.sendMail({
    ...
    if (typeof console !== 'undefined') {
      console.log('[nabd-mail] sent ' + JSON.stringify({
        messageId: info.messageId || null,
        accepted: info.accepted || [],
        rejected: info.rejected || [],
        response: info.response || null,
        envelopeFrom: info.envelope && info.envelope.from || null,
        envelopeTo: info.envelope && info.envelope.to || null,
        messageTime: info.messageTime != null ? info.messageTime : null,
        messageSize: info.messageSize != null ? info.messageSize : null
      }));
    }
